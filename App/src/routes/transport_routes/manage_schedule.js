If the server logs for vehicle number are not appearing, it suggests that the event listener for the vehicle number input field might not be working as expected. Let's ensure that the event listener is correctly set up and debug the client-side code to verify that the input events are being captured.
Updated Client-Side JavaScript with Additional Debugging
Let's add more detailed logging to verify that the event listener for the vehicle number input field is working and that the fetch request is being made.
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchAndPopulateSuggestions() {
        ...
        vehicleNoInput.addEventListener("input", function () {
            ...
            console.log(`Vehicle No Input Event Listener Triggered: ${query}`);
            if (query.length >= 0) {
                console.log(`Fetching driver details for query: ${query}`);
        ...
        routeInput.addEventListener("input", function () {
            const query = this.value;
            console.log(`Fetching route details for query: ${query}`);
            if (query.length >= 0) {
        ...
        shiftInput.addEventListener("input", function () {
            const query = this.value;
            console.log(`Fetching shift details for query: ${query}`);
            if (query.length >= 0) {
    ...
    vehicleNoSuggestionBox.addEventListener("click", function (event) {
        if (event.target.classList.contains("suggestion-item")) {
            ...
            console.log('Selected driver details:', selectedDetails);
