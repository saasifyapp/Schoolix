...
router.post('/get-attendance-summary', async (req, res) => {
    try {
        ...
        SELECT 
            'Unavailable' AS user_id, 
            'Unavailable' AS name, 
            'Visitor' AS section, 
            'Unavailable' AS standard_division, 
            visitor_logs.date_of_attendance,
            MIN(visitor_logs.in_time) AS in_time,
            MAX(visitor_logs.in_time) AS out_time,
            NULL AS in_image, -- Assuming visitors do not have image data
            NULL AS out_image  -- Assuming visitors do not have image data
        FROM attendance_user_logs AS visitor_logs
        WHERE visitor_logs.section = 'Visitor'
        GROUP BY visitor_logs.date_of_attendance;
        `;
